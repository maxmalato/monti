...
export default function CartPage() {
    ...
    return (
        <main className="flex flex-col gap-10 items-center my-4">
            {cart.length === 0 ? (
                <p className="text-center">Seu carrinho está vazio.</p>
            ) : (
                <ul className="flex flex-col items-center gap-2 w-full px-2 md:grid md:grid-cols-2 md:justify-items-center lg:grid-cols-3">
                    {cart.map((item) => (
                        <li key={item.id} className="border p-3 rounded-lg w-full">
                            ...
                            <div className="mt-4 flex justify-center gap-10">
                                <TooltipProvider>
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <Button onClick={() => removeFromCart(item.id)} variant="destructive" className="w-20">
                                                <Trash2 />
                                            </Button>
                                        </TooltipTrigger>
                                        <TooltipContent>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="bg-slate-900 text-white rounded-full p-1 cursor-pointer">
                                        <Plus size={20} />
                                    </div>
                                </div>
                            </div>
